...
namespace DataRepositories
{
    public interface IDataRepository
    {
        ...
        Task<SingleItemCrudResponse<T>> ReadSingleItem<T>(string databaseName, Func<T, bool> predicate) where T : class, new();
        Task<ItemResponse<T>> UpdateSingleItem<T, K>(string databaseName, string id, T item, K partitionKeyValue) where T : class, new();
        Task<ItemResponse<T>> DeleteSingleItem<T, K>(string databaseName, string id, K partitionKeyValue) where T : class, new();
